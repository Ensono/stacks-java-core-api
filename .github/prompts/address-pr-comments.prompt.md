...
This prompt guides an AI agent to review and address all **unresolved** comments on a GitHub Pull Request, implement the requested changes, and commit the fixes. **Individual replies MUST be posted to each comment thread** to facilitate granular review resolution.
...
3. **Filter to unresolved threads only**:
   - `isResolved: false`
   - `isOutdated: false` to skip comments on old code
...
3. **Validate the change**:
   ...
   - Run relevant tests: `./mvnw clean test`
   - Ensure no formatting or linting issues
...
**CRITICAL**: You MUST reply to each individual review thread separately using its unique `comment_id`. DO NOT post a single summary comment to the PR as a whole. Each addressed concern must have its own direct reply in the relevant thread to ensure reviewers can resolve them individually.
...
| ------- | ---------------------- | ------------------ |
| abc1234 | src/lib.java           | #1, #3             |
| def5678 | tests/integration.java | #2                 |
...
3. Categorizing comments:
   - Comment 1: Code change needed in src/routing.java:142
